...
-- WITH DEPT_AVG AS (SELECT DEPTNO, AVG(SAL) AS "평균급여" FROM EMP GROUP BY DEPTNO)
--     SELECT EMP.DEPTNO AS"부서번호별", EMP.JOB AS"직업별", AVG(SAL) AS"평균급여", COUNT(*) AS"사원수" FROM EMP
--     JOIN DEPT_AVG ON EMP.DEPTNO=DEPT_AVG.DEPTNO GROUP BY EMP.DEPTNO, EMP.JOB ORDER BY EMP.DEPTNO ASC;
